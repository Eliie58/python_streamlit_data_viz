...
def get_geopoints(chart_type):
    if chart_type == "District":
        with urlopen(
                'https://raw.githubusercontent.com/martgnz/bcn-geodata/master/districtes/districtes.geojson') as resp:
            return json.load(resp)
    else:
        with urlopen('https://raw.githubusercontent.com/martgnz/bcn-geodata/master/barris/barris.geojson') as resp:
            return json.load(resp)
...
st.plotly_chart(victim_distribution_fig)
...
with st.expander('See Explanation:'):
    ...
    st.markdown('The raw accident count per District or Neighborhood may be misleading. To get a more proportional '
                'view, we will use Number of Accidents, per 1000 residents.')
    st.markdown('##### By District')
...
st.plotly_chart(fig_scatter_victims)
